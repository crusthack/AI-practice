...
public class AsyncUtilsTests
{
    ...
    [Fact]
    public async Task WhenAllThrottled_RejectsInvalidConcurrency()
    {
        var factories = new[] { () => Task.FromResult(1) };

        await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() =>
            AsyncUtils.WhenAllThrottled(factories, maxConcurrency: 0));
    }
    ...
    [Fact]
    public async Task RetryAsync_DoesNotRetryCancellation()
    {
        int attempts = 0;

        await Assert.ThrowsAsync<OperationCanceledException>(() =>
            AsyncUtils.RetryAsync<int>(
                () =>
                {
                    attempts++;
