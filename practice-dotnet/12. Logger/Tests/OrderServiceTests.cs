...
public class OrderServiceTests
{
    ...
    public void ProcessOrder_ValidOrder_LogsStartAndComplete()
    {
        ...
        Assert.Contains(logger.Entries, e => e.Scopes.Any(ScopeContainsOrder));
    ...
    private static bool ScopeContainsOrder(object scope) =>
        scope is IEnumerable<KeyValuePair<string, object>> values &&
        values.Any(kv => kv.Key == "OrderId" && kv.Value is int id && id == 1001);
}
