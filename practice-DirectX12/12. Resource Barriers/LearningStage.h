...
{
    float ClearColor[4] = { 0.03f, 0.12f, 0.08f, 1.0f };
...
inline std::wstring StageShaderPath()
{
    ...
    {
        return L"12. Resource Barriers\\shaders\\Resource Barriers.hlsl";
    }
    std::wstring path(exePath, length);
    const size_t slash = path.find_last_of(L"\\/");
    return slash == std::wstring::npos ? L"12. Resource Barriers\\shaders\\Resource Barriers.hlsl" : path.substr(0, slash + 1) + L"shaders\\Resource Barriers.hlsl";
}
...
inline void ApplyStageSpecificSetup(LearningStageState& stage, ID3D12Device* device)
{
    ...
    const Vertex vertices[] = {
        { { -0.62f, 0.46f, 0.0f }, { 0.0f, 0.0f } },
        { { 0.62f, 0.46f, 0.0f }, { 1.0f, 0.0f } },
        { { 0.62f, -0.46f, 0.0f }, { 1.0f, 1.0f } },
        { { -0.62f, -0.46f, 0.0f }, { 0.0f, 1.0f } },
    };
    ...
    for (UINT y = 0; y < textureHeight; ++y)
    {
        ...
        {
            const bool diagonal = ((x + y) / 8) % 2 == 0;
            const bool vertical = (x / 16) % 2 == 0;
            pixels[y * textureWidth + x] = diagonal ? (vertical ? 0xff4be0a3 : 0xff1f8f5f) : 0xff0b2f24;
        }
    }
    ...
    D3D12_RESOURCE_BARRIER copyToShader = {};
    copyToShader.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
    copyToShader.Transition.pResource = stage.Texture.Get();
    copyToShader.Transition.StateBefore = D3D12_RESOURCE_STATE_COPY_DEST;
    copyToShader.Transition.StateAfter = D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
    copyToShader.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
    list->ResourceBarrier(1, &copyToShader);
