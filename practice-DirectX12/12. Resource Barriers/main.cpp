...
{
    const wchar_t* className = L"Dx1212ResourceBarriersWindowClass";
...
void Render(Dx12Context& dx, LearningStageState& stage)
{
    ...
    stageContext.Width = WindowWidth;
    stageContext.Height = WindowHeight;
...
int Run(HINSTANCE instance, int showCommand)
{
    ...
    setupContext.Device = dx.Device.Get();
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int showCommand)
{
    ...
    catch (const std::exception& ex)
    {
        ...
        MessageBoxW(nullptr, message.c_str(), L"12. Resource Barriers Error", MB_OK | MB_ICONERROR);
        return -1;
    }
}
