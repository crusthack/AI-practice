...
{
    const wchar_t* className = L"Vulkan02HelloTriangleWindowClass";
...
void InitVulkan(VulkanContext& vk)
{
    ...
    appInfo.pApplicationName = "02. HelloTriangle";
...
int Run(HINSTANCE instance, int showCommand)
{
    ...
    {
        InitVulkan(vk);
        ApplyStageSpecificSetup(stage, vk.Device, vk.RenderPass, vk.SwapchainExtent);
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int showCommand)
{
    ...
    catch (const std::exception& ex)
    {
        ...
        MessageBoxW(nullptr, message.c_str(), L"02. HelloTriangle Error", MB_OK | MB_ICONERROR);
        return -1;
    }
}
