...
// Learning goal: Create descriptor set layouts, descriptor pools, and bind uniform descriptors.
...
struct LearningStageState
{
    ...
    VkBuffer TransformUniformBuffer = VK_NULL_HANDLE;
    VkDeviceMemory TransformUniformMemory = VK_NULL_HANDLE;
    void* TransformUniformMapped = nullptr;
    VkBuffer TintUniformBuffer = VK_NULL_HANDLE;
    VkDeviceMemory TintUniformMemory = VK_NULL_HANDLE;
    void* TintUniformMapped = nullptr;
...
inline void CreateDescriptorResources(LearningStageState& stage, VkPhysicalDevice physicalDevice, VkDevice device)
{
    ...
    CreateBuffer(physicalDevice, device, sizeof(UniformData), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stage.TransformUniformBuffer, stage.TransformUniformMemory);
    StageThrowIfFailed(vkMapMemory(device, stage.TransformUniformMemory, 0, sizeof(UniformData), 0, &stage.TransformUniformMapped), "vkMapMemory failed.");

    CreateBuffer(physicalDevice, device, sizeof(TintData), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stage.TintUniformBuffer, stage.TintUniformMemory);
    StageThrowIfFailed(vkMapMemory(device, stage.TintUniformMemory, 0, sizeof(TintData), 0, &stage.TintUniformMapped), "vkMapMemory failed.");
    ...
    bufferInfos[0].buffer = stage.TransformUniformBuffer;
    bufferInfos[0].range = sizeof(UniformData);
    bufferInfos[1].buffer = stage.TintUniformBuffer;
    bufferInfos[1].range = sizeof(TintData);

    VkWriteDescriptorSet descriptorWrites[2] = {};
    descriptorWrites[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    descriptorWrites[0].dstSet = stage.DescriptorSet;
    descriptorWrites[0].dstBinding = 0;
    descriptorWrites[0].descriptorCount = 1;
    descriptorWrites[0].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    descriptorWrites[0].pBufferInfo = &bufferInfos[0];
    descriptorWrites[1].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    descriptorWrites[1].dstSet = stage.DescriptorSet;
...
{
    const auto vertexShaderCode = ReadShaderBinary(L"shaders/Descriptor Sets.vert.spv");
    const auto fragmentShaderCode = ReadShaderBinary(L"shaders/Descriptor Sets.frag.spv");
...
inline void UpdateStageSpecificDemo(LearningStageState& stage, double timeSeconds)
{
    ...
    std::memcpy(stage.TransformUniformMapped, &uniform, sizeof(uniform));

    TintData tint = {};
    tint.Tint[0] = 0.80f + 0.20f * std::cos(static_cast<float>(timeSeconds));
    tint.Tint[1] = 0.90f + 0.10f * std::sin(static_cast<float>(timeSeconds) * 0.7f);
    tint.Tint[2] = 1.0f;
    tint.Tint[3] = 1.0f;
    std::memcpy(stage.TintUniformMapped, &tint, sizeof(tint));
}
...
inline void ApplyStageSpecificCleanup(LearningStageState& stage)
{
    ...
    if (stage.TintUniformMapped) vkUnmapMemory(stage.Device, stage.TintUniformMemory);
    if (stage.TintUniformBuffer) vkDestroyBuffer(stage.Device, stage.TintUniformBuffer, nullptr);
    if (stage.TintUniformMemory) vkFreeMemory(stage.Device, stage.TintUniformMemory, nullptr);
    if (stage.TransformUniformMapped) vkUnmapMemory(stage.Device, stage.TransformUniformMemory);
    if (stage.TransformUniformBuffer) vkDestroyBuffer(stage.Device, stage.TransformUniformBuffer, nullptr);
    if (stage.TransformUniformMemory) vkFreeMemory(stage.Device, stage.TransformUniformMemory, nullptr);
