...
## Intent
Update shader uniforms every frame for animation and per-draw parameters.
...
## Expected Result
A 1280x720 window titled `07. Uniforms And Time` opens, clears to a dark blue-green background, and continues presenting until closed.
...
- `shaders/`: contains `Uniforms And Time.glsl` and shader policy notes.
...
```powershell
& "C:\Program Files\Microsoft Visual Studio\18\Community\MSBuild\Current\Bin\MSBuild.exe" ".\07. Uniforms And Time\07. Uniforms And Time.vcxproj" /p:Configuration=Debug /p:Platform=x64
```
