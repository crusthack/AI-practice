...
## Intent
Send model, view, and projection matrices to GLSL and transform geometry.
...
## Expected Result
A 1280x720 window titled `08. Transform Matrices` opens, clears to a dark blue-green background, and continues presenting until closed.
...
- `shaders/`: contains `Transform Matrices.glsl` and shader policy notes.
...
```powershell
& "C:\Program Files\Microsoft Visual Studio\18\Community\MSBuild\Current\Bin\MSBuild.exe" ".\08. Transform Matrices\08. Transform Matrices.vcxproj" /p:Configuration=Debug /p:Platform=x64
```
