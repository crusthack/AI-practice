...
## Intent
Use a geometry shader for controlled primitive expansion or visualization.
...
## Expected Result
A 1280x720 window titled `24. Geometry Shader` opens, clears to a dark blue-green background, and continues presenting until closed.
...
- `shaders/`: contains `Geometry Shader.glsl` and shader policy notes.
...
```powershell
& "C:\Program Files\Microsoft Visual Studio\18\Community\MSBuild\Current\Bin\MSBuild.exe" ".\24. Geometry Shader\24. Geometry Shader.vcxproj" /p:Configuration=Debug /p:Platform=x64
```
