...
## Intent
Create a procedural texture, configure sampler state, and sample it in GLSL.
...
## Expected Result
A 1280x720 window titled `10. Texture Sampling` opens, clears to a dark blue-green background, and continues presenting until closed.
...
- `shaders/`: contains `Texture Sampling.glsl` and shader policy notes.
...
```powershell
& "C:\Program Files\Microsoft Visual Studio\18\Community\MSBuild\Current\Bin\MSBuild.exe" ".\10. Texture Sampling\10. Texture Sampling.vcxproj" /p:Configuration=Debug /p:Platform=x64
```
