...
## Intent
Create a cubemap texture and render a skybox with depth-state changes.
...
## Expected Result
A 1280x720 window titled `18. Cubemap Skybox` opens, clears to a dark blue-green background, and continues presenting until closed.
...
- `shaders/`: contains `Cubemap Skybox.glsl` and shader policy notes.
...
```powershell
& "C:\Program Files\Microsoft Visual Studio\18\Community\MSBuild\Current\Bin\MSBuild.exe" ".\18. Cubemap Skybox\18. Cubemap Skybox.vcxproj" /p:Configuration=Debug /p:Platform=x64
```
