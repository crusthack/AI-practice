...
// Learning goal: upload vertex data into a VBO and draw it with glDrawArrays.
// Implementation status: Implemented.
//
// The shader program remains the same GLSL 1.20 compatibility lesson from 03.
// This sample adds only one new resource type: an OpenGL array buffer.
...
struct LearningStageState
{
    ...
    float ClearColor[4] = { 0.030f, 0.045f, 0.065f, 1.0f };
};
...
inline GLuint CreateTriangleProgram(const OpenGLApi& gl)
{
...
inline GLuint CreateTriangleVertexBuffer(const OpenGLApi& gl, GLsizei& vertexCount)
{
    const Vertex vertices[] =
    {
        { { 0.0f, 0.68f }, { 0.92f, 0.24f, 0.18f } },
        { { -0.72f, -0.56f }, { 0.20f, 0.66f, 1.0f } },
        { { 0.72f, -0.56f }, { 0.28f, 0.92f, 0.42f } },
    };
    ...
    vertexCount = static_cast<GLsizei>(sizeof(vertices) / sizeof(vertices[0]));
    return buffer;
}
...
inline void ApplyStageSpecificSetup(LearningStageState& stage, const LearningStageSetupContext& context)
{
    ...
    stage.VertexBuffer = CreateTriangleVertexBuffer(stage.Gl, stage.VertexCount);
...
inline void ApplyStageSpecificRender(LearningStageState& stage, const LearningStageRenderContext& context)
{
    ...
    glDrawArrays(GL_TRIANGLES, 0, stage.VertexCount);
