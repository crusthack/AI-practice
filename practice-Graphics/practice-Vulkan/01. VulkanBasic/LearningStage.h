...
// Learning goal: Create a Win32 window, Vulkan instance, surface, device, swapchain, command buffers, and clear/present loop.
...
inline void ApplyStageSpecificSetup(LearningStageState& stage, VkDevice device)
{
    (void)stage;
    (void)device;
    // The first sample intentionally owns no extra stage resources; it only clears the swapchain image.
}
