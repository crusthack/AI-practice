...
inline void CreateGeometryBuffers(
    ...
    VkQueue queue)
{
    ...
    UploadToDeviceLocalBuffer(
        ...
        stage.IndexBuffer,
        stage.IndexBufferMemory);
}

inline void CreateGraphicsPipeline(LearningStageState& stage, VkDevice device, VkRenderPass renderPass, VkExtent2D extent)
{
    const std::vector<char> vertexShaderCode = ReadShaderBinary(L"shaders/Index Buffer.vert.spv");
    const std::vector<char> fragmentShaderCode = ReadShaderBinary(L"shaders/Index Buffer.frag.spv");
...
inline void ApplyStageSpecificCleanup(LearningStageState& stage)
{
    ...
    if (stage.IndexBuffer)
    {
        vkDestroyBuffer(stage.Device, stage.IndexBuffer, nullptr);
        stage.IndexBuffer = VK_NULL_HANDLE;
    }

    if (stage.IndexBufferMemory)
    {
        vkFreeMemory(stage.Device, stage.IndexBufferMemory, nullptr);
        stage.IndexBufferMemory = VK_NULL_HANDLE;
    }
