...
## Intent
Render depth from a light view and sample it when shading the main scene.
...
## Expected Result
A 1280x720 window titled `23. Shadow Mapping` opens, clears to a dark blue-green background, and continues presenting until closed.
...
- `shaders/`: contains `Shadow Mapping.glsl` and shader policy notes.
...
```powershell
& "C:\Program Files\Microsoft Visual Studio\18\Community\MSBuild\Current\Bin\MSBuild.exe" ".\23. Shadow Mapping\23. Shadow Mapping.vcxproj" /p:Configuration=Debug /p:Platform=x64
```
