...
## Intent
Dispatch a compute shader and synchronize its result for rendering or readback.
...
## Expected Result
A 1280x720 window titled `25. Compute Shader` opens, clears to a dark blue-green background, and continues presenting until closed.
...
- `shaders/`: contains `Compute Shader.glsl` and shader policy notes.
...
```powershell
& "C:\Program Files\Microsoft Visual Studio\18\Community\MSBuild\Current\Bin\MSBuild.exe" ".\25. Compute Shader\25. Compute Shader.vcxproj" /p:Configuration=Debug /p:Platform=x64
```
