...
## Intent
Build an orbit camera and update the view matrix from keyboard input.
...
## Expected Result
A 1280x720 window titled `09. Camera` opens, clears to a dark blue-green background, and continues presenting until closed.
...
- `shaders/`: contains `Camera.glsl` and shader policy notes.
...
```powershell
& "C:\Program Files\Microsoft Visual Studio\18\Community\MSBuild\Current\Bin\MSBuild.exe" ".\09. Camera\09. Camera.vcxproj" /p:Configuration=Debug /p:Platform=x64
```
