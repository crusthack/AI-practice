...
## Intent
Draw a fullscreen pass from an FBO texture and apply a simple effect.
...
## Expected Result
A 1280x720 window titled `15. Post Processing` opens, clears to a dark blue-green background, and continues presenting until closed.
...
- `shaders/`: contains `Post Processing.glsl` and shader policy notes.
...
```powershell
& "C:\Program Files\Microsoft Visual Studio\18\Community\MSBuild\Current\Bin\MSBuild.exe" ".\15. Post Processing\15. Post Processing.vcxproj" /p:Configuration=Debug /p:Platform=x64
```
