...
## Intent
Store related textures in a texture array and choose layers in the shader.
...
## Expected Result
A 1280x720 window titled `17. Texture Array` opens, clears to a dark blue-green background, and continues presenting until closed.
...
- `shaders/`: contains `Texture Array.glsl` and shader policy notes.
...
```powershell
& "C:\Program Files\Microsoft Visual Studio\18\Community\MSBuild\Current\Bin\MSBuild.exe" ".\17. Texture Array\17. Texture Array.vcxproj" /p:Configuration=Debug /p:Platform=x64
```
