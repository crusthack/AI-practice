...
// File: main.cpp
//
// Example 3: Shader Color Interpolation
// - Extends Example 2 by adding a COLOR vertex attribute.
// - Uses a dynamic vertex buffer when CPU-side color animation is needed.
//--------------------------------------------------------------------------------------
#include <windows.h>
#include <d3d11.h>
...
PS_INPUT VS(VS_INPUT input)
{
    ...
    // COLOR is passed to the rasterizer. Pixels between vertices receive interpolated values.
...
HRESULT InitWindow( HINSTANCE hInstance, int nCmdShow )
{
    ...
    g_hWnd = CreateWindow( L"TutorialWindowClass", L"Direct3D 11 Tutorial 3: Shader Color Interpolation",
...
HRESULT InitDevice()
{
    ...
    {
        { XMFLOAT3( 0.0f, 0.65f, 0.5f ), XMFLOAT4( 1.0f, 0.1f, 0.1f, 1.0f ) },
    ...
    // DYNAMIC + CPU_ACCESS_WRITE lets Map/Unmap update the buffer each frame.
    // This pattern is a preview of per-frame GPU uploads; Example 5 formalizes
    // the same idea more cleanly through constant buffers.
...
void UpdateTriangleColors()
{
    ...
    SimpleVertex vertices[] =
    {
        { XMFLOAT3( 0.0f, 0.65f, 0.5f ), XMFLOAT4( 0.5f + 0.5f * sinf( t ), 0.1f, 0.1f, 1.0f ) },
        { XMFLOAT3( 0.65f, -0.55f, 0.5f ), XMFLOAT4( 0.1f, 0.5f + 0.5f * sinf( t + 2.0f ), 0.1f, 1.0f ) },
        { XMFLOAT3( -0.65f, -0.55f, 0.5f ), XMFLOAT4( 0.1f, 0.25f, 0.5f + 0.5f * sinf( t + 4.0f ), 1.0f ) },
    };
    ...
    // Dynamic resources are updated by mapping existing memory, not by creating a new buffer every frame.
...
void Render()
{
    ...
    float clearColor[4] = { 0.02f, 0.02f, 0.06f, 1.0f };
